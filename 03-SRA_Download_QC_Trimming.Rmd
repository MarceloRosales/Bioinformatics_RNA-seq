...
Whit this files will now map the reads of our samples 
...
For aligning there is some issues with the flags `command Z cat` which tells star tha the files were inputting are zipped.  
The way around it is to (maybe not the best way) gun zip, and the put on the files that we want (treammed reads) in a cat file. (file containig a catalog or list of the files downloaded or to be fed to the  application).  


```{bash, eval=FALSE}
cat Filename.txt | parallel -j 1 "gunzip trimmedReads/{}_*P.gz && 
```
